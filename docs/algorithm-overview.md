...
| **Short logic** | `LIVE` + `SHADOW` | **Side-aware micro peak-lock (2026-07-24) — two-sided trading restored.** The twins' chronic short bleed (4 negative weeks, −$228) was mechanical: their 3m-timed shorts peak ~1.2% ROE — just under TP1, the 1.5 breakeven arm, and the 1.2 micro-lock arm — so 42% round-tripped to the full stop. Threshold sweep on 334 real shorts (Jul 14–24, deltas vs live): **arm 0.6 / floor 0.5 for shorts** improved the twins +$81.5/10d AND the other cohorts +$208.1/10d (locks 47→80, no damage to the profit-fade harvest); shipped side-aware, longs unchanged (1.2/0.35). The Jul-21 side-off A/B is closed (Kush's control bar: n=35, −$16.79 — mild-negative; the fix replaced the side kill). If the twins' shorts stay clearly negative over n≥25 WITH the lock, the residual is entry-side → signal-rebuild list, not side flags. **Continuation-short gate (`LIVE`, all cohorts): shorts are fade-only.** Shorting a symbol already in a bearish 24h regime (34% win, −$72/14d) or with RSI<40 (32% win) is blocked outright; only fade-shaped shorts (bullish/sideways 24h symbol, RSI≥40 — 45.6% win, +$18/14d measured) may book. On top, the **mean-reversion fade filter** (`SHADOW`, treatment cohorts) requires an overextended, overbought, volume-spiking 15m move rolling over. Backtest: 56–58% win / PF 1.5–1.7. |
