...
| **Entry gates** | `LIVE` | Regime filter · spread cap · momentum-exhaustion block · per-symbol post-stop cooldown · major-coin confidence floor · min-net-profit-to-enter · blocked session hours · **2-scan entry confirmation** (a filtered long/short must clear its filter on two consecutive scans before booking, so a coin can't slip in on a single lucky 1m tick and whip to the full stop). |
