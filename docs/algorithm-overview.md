...
| **V2 confluence gate — LONGS** | `PROPOSED` (dormant) | Measured-component score replaces confidence as the long ranker: trend *strength* +2 · calm volume +1 / spike −2 · RSI 40–55 +1 / 55–65 −1 · sideways 24h +1 / bullish −1; books at score ≥ 2. Replay on 424 live trades: selected 52.6% win (+$32.74) vs rejected 35.8% (−$87.86); separates even within structure-filter survivors (43.8% vs 33.6%). Shipped dormant (`v2_long_gate_enabled`, off everywhere) pending a 5–7 day out-of-sample window; flips by config, no deploy. |
...
6. **Side-aware stop geometry (July-19 agenda):** SHORTS want the opposite scale from longs — the fade-short sweep was monotonic: tightest geometry least-bad (0.85/1.45 price → PF 0.90) while the long-side scale was catastrophic (1.5/2.55 → PF 0.61, 26% win). Live shorts currently inherit the LONG preset geometry (computeRiskPlan is side-agnostic) — i.e., they run at the measured-worst width. Candidate: shorts get ~1.0/1.5 price geometry. Also: the mean-rev fade is regime-cyclical (56–58% win in the prior pump week, negative in this soft week — few blow-offs to fade); judge with live A/B data, don't widen anything.
