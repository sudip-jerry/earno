...
| **Entry gates** | `LIVE` | Regime filter · spread cap · momentum-exhaustion block · per-symbol post-stop cooldown · major-coin confidence floor · min-net-profit-to-enter · blocked session hours · **2-scan entry confirmation — all cohorts, both sides, final gate**: an entry must survive every other gate on two consecutive scans before booking (kills the single-lucky-tick whipsaw entry); the DB counter enforces a min-gap so overlapping scan passes can't double-count. A **1-minute hot-list pass** (odd minutes; full scan runs even minutes) re-checks ONLY awaiting-confirmation candidates through the identical gate chain, so the 2nd look lands ~60s after the 1st instead of ~120s — measured cost of the old wait: median 0.048% price adverse drift per booking (28/38 moved against). The debounce itself stays: its rejected candidates averaged +0.06% gross at 30m, below the 0.118% round-trip fee, so single-look booking remains off. Per-cohort kill switch: `hotlist_enabled=false`. In bearish regimes, surviving fade shorts are held to the counter-trend floor (the old with-trend short discount was removed — it was calibrated for shorting falling symbols, which the gainers-only universe no longer contains). |
