          ...
          {tab === "open" ? (
            <div className="brand-hero rounded-2xl p-4 flex items-center justify-between shadow-md">
              ...
              <div className="text-right">
                ...
                <p className="text-2xl font-semibold tabular-nums mt-0.5 text-white">
                  {rows.length}
                </p>
              </div>
            </div>
                      ...
                      {(p.breakeven_armed_at || p.breakeven_moved) && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-sky-500/10 text-sky-500">
                          ...
                          {p.peak_unrealized_pnl_pct != null
                            ? ` · Peak +${Number(p.peak_unrealized_pnl_pct).toFixed(2)}%`
                            : ""}
                        </span>
                      )}
                      {p.tp1_hit && (
            ...
            {!q.isLoading && rows.length === 0 ? (
              <li>
                <BrandEmptyState
                  ...
                  message="earn'O opens trades automatically when it finds a good setup — check back soon."
                />
              </li>
            ) : null}
          </ul>
    ...
    return (
      <div className="px-5 mt-3">
        <BrandEmptyState
          ...
          message="Once a trade closes, it'll show up here with its result."
        />
      </div>
    );
  }
