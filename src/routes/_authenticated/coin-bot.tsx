...
import {
  ...
  getCoinConfig, updateCoinConfig,
...
export const Route = createFileRoute("/_authenticated/coin-bot")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Coin buy/sell paper trading using real CoinDCX public market data." },
    ],
  }),
...
const ACTION_LABEL: Record<Action, string> = {
  buy: "Buy", sell: "Sell", hold: "Hold", wait: "Wait", avoid: "Avoid",
};
...
function CoinBotPage() {
  ...
  const signals = useQuery({ queryKey: ["coin_signals"], queryFn: () => signalsFn(), refetchInterval: 30_000 });
  ...
  const buy = useMutation({
    ...
    onSuccess: (r) => {
      if ((r as any).ok) { toast.success("Paper buy booked"); refreshAll(); }
  ...
  const sell = useMutation({
    ...
    onSuccess: (r) => {
      if ((r as any).ok) { toast.success("Paper sell booked"); refreshAll(); }
  ...
  const updCfg = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["coin_cfg"] }); qc.invalidateQueries({ queryKey: ["coin_portfolio"] }); toast.success("Settings saved"); },
  });
    ...
    <div className="min-h-dvh pb-24 bg-background text-foreground">
      <header className="sticky top-0 z-20 border-b bg-background/95 backdrop-blur">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="ghost" onClick={() => setShowCfg((v) => !v)}><Cog className="size-4" /></Button>
      ...
      <main className="max-w-md mx-auto px-4 py-4 space-y-4">
        ...
        <section className="rounded-2xl border bg-card p-4">
          <div className="text-xs uppercase tracking-wide text-muted-foreground">Coin portfolio</div>
        ...
        {showCfg && c && (
          <section className="rounded-2xl border bg-card p-4 space-y-3">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <Button size="sm" variant={c.enabled ? "default" : "outline"} onClick={() => updCfg.mutate({ enabled: !c.enabled })}>
                {c.enabled ? "On" : "Off"}
              </Button>
            </div>
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex gap-1">
                <Button size="sm" variant={c.mode === "intraday" ? "default" : "outline"} onClick={() => updCfg.mutate({ mode: "intraday" })}>Intraday</Button>
                <Button size="sm" variant={c.mode === "swing" ? "default" : "outline"} onClick={() => updCfg.mutate({ mode: "swing" })}>Swing</Button>
              </div>
            </div>
            <div className="text-xs text-muted-foreground">
              Intraday: scans every {c.scan_interval_min}m, prefers short-term momentum, can carry overnight if trend valid.
              Swing: holds across days, max {c.max_holding_days} days. No forced time-based exits.
            </div>
          </section>
        )}
          ...
          {h?.open?.length ? (
            <div className="overflow-hidden rounded-2xl border bg-card">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {h.open.map((row) => {
                    const pnlPct = ((row.last_price! - row.avg_buy_price) / row.avg_buy_price) * 100;
                    return (
                      <tr key={row.id} className="border-t">
                        ...
                        <td className="px-2 py-2 text-right">
                          <Button size="sm" variant="outline" onClick={() => sell.mutate({ positionId: row.id, price: row.last_price ?? row.avg_buy_price })}>Sell</Button>
                        </td>
                      </tr>
                    );
                  })}
            ...
            <div className="rounded-2xl border bg-card p-4 text-sm text-muted-foreground">No active coin holdings yet.</div>
          )}
        </section>

        {/* Signals */}
        <section>
          <div className="px-1 pb-2 text-xs uppercase tracking-wide text-muted-foreground">Live coin signals</div>
          {sigs.length === 0 ? (
              ...
              {sigs.map((s) => (
                <li key={s.id} className="rounded-2xl border bg-card p-3">
                  ...
                  <div className="mt-2 flex gap-2">
                    ...
                    {s.action === "sell" && (
                      <Button size="sm" variant="outline" onClick={() => sell.mutate({ positionId: "", price: s.price })} disabled>
