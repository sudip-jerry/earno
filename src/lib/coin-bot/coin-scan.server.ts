...
export async function runCoinScanFor(
    ...
    await Promise.all(batch.map(async (t) => {
      try {
        ...
        signalsToInsert.push({
          user_id: userId, symbol: t.symbol, display: t.display,
  ...
  await supabase.from("coin_signals").update({ status: "superseded" })
  ...
  for (const sig of signalsToInsert) {
    ...
    const { data: row } = await supabase.from("coin_positions").select("*")
    ...
    await supabase.from("coin_positions").update({
  ...
  if (cfg.enabled) {
    ...
    for (const s of buys.slice(0, slots)) {
      ...
      await supabase.from("coin_positions").insert({
        user_id: userId, symbol: s.symbol, display: s.display,
  ...
  if (cashDelta !== 0) {
    await supabase.from("coin_bot_config").update({
