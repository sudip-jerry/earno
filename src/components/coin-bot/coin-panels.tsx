    ...
    <section className="rounded-2xl border bg-card p-4">
      ...
      <div className="mt-2 grid grid-cols-2 gap-3 text-sm">
        ...
        <Stat
          ...
          value={fmtCur(Number(p?.realized_today_usdt ?? 0), { signed: true })}
        />
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 text-xs text-muted-foreground">
          <div>
            ...
            <span className="text-foreground">
              ...
              {h.summary.best_pnl_pct != null ? `(${fmtRaw(h.summary.best_pnl_pct)}%)` : ""}
            </span>
          </div>
          <div>
            Worst:{" "}
            <span className="text-foreground">
              ...
              {h.summary.worst_pnl_pct != null ? `(${fmtRaw(h.summary.worst_pnl_pct)}%)` : ""}
            </span>
          </div>
        </div>
      )}
    ...
    <div className="overflow-hidden rounded-2xl border bg-card">
      <table className="w-full text-xs">
        ...
        <tbody>
          {open.map((row) => {
            const pnlPct =
              ((Number(row.last_price) - Number(row.avg_buy_price)) / Number(row.avg_buy_price)) *
              100;
            return (
...
export function CoinSignalsList({
  ...
  // "Top signals" surfaces only actionable opportunities (buy/sell) — hold / wait /
  // avoid are noise there, matching how the futures side never lists "avoid".
  if (actionableOnly) sigs = sigs.filter((s) => s.action === "buy" || s.action === "sell");
          ...
          {sigs.map((s) => (
            <li key={s.id} className="rounded-2xl border bg-card p-3">
              ...
              <div className="mt-2 grid grid-cols-3 gap-2 text-[11px] text-muted-foreground">
                <div>
                  Price <span className="text-foreground">{fmtRaw(s.price, 6)}</span>{" "}
                  <span className="text-[9px]">USDT</span>
                </div>
                <div>
                  Target <span className="text-foreground">{fmtRaw(s.target, 6)}</span>{" "}
                  <span className="text-[9px]">USDT</span>
                </div>
                <div>
                  Stop <span className="text-foreground">{fmtRaw(s.stop, 6)}</span>{" "}
                  <span className="text-[9px]">USDT</span>
                </div>
              </div>
