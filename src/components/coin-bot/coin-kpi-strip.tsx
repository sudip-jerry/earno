...
export function CoinKpiStrip() {
  ...
  return (
    <section className="grid grid-cols-4 gap-2">
      ...
      <Kpi
        label="Best"
        value={sum?.best_pnl_pct != null ? `${Number(sum.best_pnl_pct).toFixed(1)}%` : "—"}
        sub={sum?.best_symbol ?? undefined}
        tone="pos"
      />
      <Kpi
        label="Worst"
        value={sum?.worst_pnl_pct != null ? `${Number(sum.worst_pnl_pct).toFixed(1)}%` : "—"}
        sub={sum?.worst_symbol ?? undefined}
        tone="neg"
      />
    </section>
  );
